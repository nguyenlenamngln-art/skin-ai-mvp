...
app=FastAPI(title='Skin AI Product API',version='0.4.1')
...
def health():
    return {
        'status':'ok',
        'uv_model_available':MODEL_PATH.exists(),
        'model_available':MODEL_PATH.exists(),
        'rgb_engine_available':True,
        'rgb_engine_version':RGBAnalysisEngine.VERSION,
...
def list_scans(limit:int=30, modality:str|None=None):
    if modality not in (None,'uv','rgb'):
        raise HTTPException(status_code=400,detail='modality must be uv or rgb')
...
async def analyze_rgb(image:UploadFile=File(...)):
    rgb=decode_image(await image.read())
    try:
        result=get_rgb_engine().analyze_rgb(rgb)
